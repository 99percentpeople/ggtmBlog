        ...
        >
            <template #header>
                <n-h3 style="display: flex; align-items: center; margin: 0">
                    <n-icon> <List24Regular /> </n-icon>标签管理
                </n-h3>
            </template>
            <n-form ref="formRef" inline :model="formValue" size="medium" label-placement="left">
                ...
                <n-form-item>
                    <n-button type="info" @click="onAddTags">添加</n-button>
                </n-form-item>
            </n-form>
            <n-data-table
                remote
...
import { deleteTag, getTagsList, postOneTag, putTag } from "@/apis";
...
function createTagsList(size: number, index: number) {
    loadingRef.value = true;
    return getTagsList(size, index, tagQuery)
        .then(({ data }) => {
...
onMounted(() => {
    createTagsList(pagination.pageSize, pagination.page);
});
function onAddTags(e: MouseEvent) {
    e.preventDefault();
    formRef.value?.validate((errors) => {
        if (!errors) {
            postOneTag(formValue.value.name)
                .then((data) => {
                    message.success(data.msg);
                    dataRef.value.push(data.data);
                    formValue.value.name = "";
                    return createTagsList(pagination.pageSize, pagination.page);
                })
                .catch((error) => {
    ...
    delete(rowData: RowData) {
        deleteTag(rowData.id)
            .then((data) => {
                ...
                return createTagsList(pagination.pageSize, pagination.page).then(() => {
...
const pagination = reactive({
    ...
    onChange: (index: number) => {
        if (!loadingRef.value) {
            createTagsList(pagination.pageSize, index).then(() => {
                pagination.page = index;
            });
        }
    },
    onUpdatePageSize: (pageSize: number) => {
        createTagsList(pageSize, pagination.page).then(() => {
